...
5. Click the **gear icon → Project settings** (top-left).
6. Scroll to **Your apps** and click the **web icon** `</>`.
7. Give it a nickname (e.g. `bingo-web`) and click **Register app**. You do **not** need Firebase Hosting here.
...
5. Players watch and tap the matching tiles on their card. The two biases to look for show **above the video**.
6. Press **■ End ad** when you want to stop it, then move to the **Next** ad.
7. Watch the **Leaderboard** fill up. Because every card is uniquely shuffled, lines complete for only some players at a time — so winners stand out.
...
In **Add an ad now**, paste any YouTube link (or the 11-character video ID), give it a title, pick 2 (or 3) biases from the dropdowns, and click **Add to playlist**. It appears instantly in your playlist, ready to play. The biases you chose are what players see above that video.
